...
class Wp_popular_posts extends WP_Widget {
	...
	function Wp_popular_posts() {
		$widget_ops  = array( 'classname' => 'widgets_popular_post', 'description' => __( 'Display popular articles post type.', 'asli_bejo' ) );
		$control_ops = array( 'id_base' => 'widgets_popular_post' );

		parent::__construct( 'widgets_popular_post', __( 'Widgets Popular Posts', 'asli_bejo' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$widgets_popular_post_title = apply_filters('widget_title', empty( $instance['widgets_popular_post_title'] ) ? __( '', 'asli_bejo' ) : $instance['widgets_popular_post_title'], $instance, $this->id_base );
		...
		echo $before_widget;
        	echo $before_title . $widgets_popular_post_title . $after_title; ?>
		 	<div class="inner-widget">
				<ul>
				    ...
				    if ( $wp_query->have_posts() ) : while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
						<li>
			    			<article class="popular-post clearfix">
								...
								<div class="detail">
									<h3 class="post-title"><a href="<?php the_permalink(); ?>"><?php echo wp_trim_words( get_the_title(), $my_wp_popular_title_limiter .' ...' ); ?></a></h3>
					...
					<?php endwhile; else: _e( 'Not have popular posts !', 'asli_bejo' ); endif; ?>
				</ul>
			</div>
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'widgets_popular_post_title' => __( 'Popular Posts', 'asli_bejo' ), 'widgets_popular_post_count' => '3', 'my_wp_popular_title_limiter' => '10') );
	?>
        <p>
            <label for="<?php echo $this->get_field_id( 'widgets_popular_post_title' ); ?>"><?php _e( 'Widget Title:', 'asli_bejo' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'widgets_popular_post_count' ); ?>"><?php _e('Number of posts to show:', 'asli_bejo'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'my_wp_popular_title_limiter' ); ?>"><?php _e('Post Title Limiter', 'asli_bejo'); ?></label>
            ...
            <p><small><?php _e( 'The post title will be trim after reaching the number of characters defined.', 'asli_bejo' ); ?></small></p>
        </p>
	<?php
	}
}
